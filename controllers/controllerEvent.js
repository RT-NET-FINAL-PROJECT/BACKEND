...
class ControllerEvent {
  static async findAllEvent(req, res, next) {
    ...
    static async createEvent(req, res, next) { //aman
        try {
            const { name, deskripsi, category, lokasi, biaya } = req.body;
            if (!name || !deskripsi || !category || !lokasi || !biaya) throw { name: "NO_INPUT" }
    ...
    static async updateEvent(req, res, next) { //aman
        try {
            ...
            if (!name || !deskripsi || !category || !lokasi || !biaya)
                throw { name: "NO_INPUT" };
            ...
            await Post.update(
                { name, deskripsi, kategori:category, lokasi, biaya },
                { where: { id: event.id } }
            );
